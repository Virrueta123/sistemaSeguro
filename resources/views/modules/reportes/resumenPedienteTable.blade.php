<table class="table" border="1">
    <thead>
        <tr>
            <th colspan="22"
                style="font-size:22px; margin:6px; background-color: #0082FC; border:3px solid rgb(0, 14, 76); color:white; text-align: center; padding: 4px;">
                <h1>Padron Sunat de las fechas : {{ \Carbon\Carbon::parse($fechaI)->format('d/m/Y') }} hasta
                    {{ \Carbon\Carbon::parse($fechaF)->format('d/m/Y') }}</h1>
            </th>
        </tr>
        <tr>
            <th
                style="background-color: #0082FC; border:3px solid rgb(0, 14, 76); color:white; text-align: center; padding: 4px;">
                RUC DEL INFORMANTE</th>
            <th
                style="background-color: #0082FC; border:3px solid rgb(0, 14, 76); color:white; text-align: center; padding: 4px;">
    ...
    <tbody>
        ...
        @foreach ($padronSunats as $padronSunat)
            <tr>
                ...
                <td style="border:3px solid rgb(0, 14, 76); text-align: center;">
                    {{ explode(' ', $padronSunat->Prx_Apellido)[0] }}</td>

                <td style="border:3px solid rgb(0, 14, 76); text-align: center;">
                    {{ explode(' ', $padronSunat->Prx_Apellido)[1] }}</td>
                ...
                <td style="border:3px solid rgb(0, 14, 76); text-align: center;">SAN MARTIN</td>
                <td style="border:3px solid rgb(0, 14, 76); text-align: center;">{{ $padronSunat->Prx_Direccion }}
                </td>
                <td style="border:3px solid rgb(0, 14, 76); text-align: center;">{{ $padronSunat->Prx_Contacto }}</td>
                <td style="border:3px solid rgb(0, 14, 76); text-align: center;">{{ $padronSunat->Prx_VigenciaI }}
                </td>
                <td style="border:3px solid rgb(0, 14, 76); text-align: center;">{{ $padronSunat->Prx_NroCat }}</td>
                <td style="border:3px solid rgb(0, 14, 76); text-align: center;">
                    {{ \Carbon\Carbon::parse($padronSunat->Prx_Vigencia)->format('Y') }}</td>
                <td style="border:3px solid rgb(0, 14, 76); text-align: center;">S/
                    {{ moneyformat($padronSunat->Csx_MontoCat + $padronSunat->Csx_GananciaCat) }}</td>
        ...
        <tr>
            ...
            <td></td>
            <td></td>
            <td style="background-color: #0082FC; border:3px solid rgb(0, 14, 76); color:white; text-align: center;">S/
